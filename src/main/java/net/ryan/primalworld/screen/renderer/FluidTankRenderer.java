package net.ryan.primalworld.screen.renderer;
...
public class FluidTankRenderer {
    ...
    private static final int MIN_FLUID_HEIGHT = 4; // ensure tiny amounts of fluid are still visible
    ...
    private void drawFluid(GuiGraphics guiGraphics, final int width, final int height, FluidStack fluidStack) {
        ...
        if (fluid.isSame(Fluids.EMPTY)) {
            System.out.println("Fluid is empty!");
            return;
        }

        // Obtenha a textura do fluido
        TextureAtlasSprite fluidStillSprite = getStillFluidSprite(fluidStack);
        if (fluidStillSprite == null) {
            System.out.println("Fluid sprite not found!");
            return;
        }

        // Obtenha a cor do fluido
        int fluidColor = getColorTint(fluidStack);

        // Calcular a altura escalada do fluido
        long amount = fluidStack.getAmount();
        long scaledAmount = (amount * height) / capacity;

        // Log para verificar a quantidade e a altura escalada
        System.out.println("Fluid amount: " + amount);
        System.out.println("Scaled fluid height: " + scaledAmount);

        // Garantir que a altura do fluido seja visível
    ...
    private TextureAtlasSprite getStillFluidSprite(FluidStack fluidStack) {
        ...
        // Obtenha a textura ainda do fluido
        ResourceLocation fluidStill = renderProperties.getStillTexture(fluidStack);

        if (fluidStill == null) {
            System.out.println("No still texture found for fluid: " + fluid);
            return null;
        }

        // Retorna o sprite da textura do fluido
    ...
    public List<Component> getTooltip(FluidStack fluidStack, TooltipFlag tooltipFlag) {
        ...
        try {
            ...
            if (tooltipMode == TooltipMode.SHOW_AMOUNT_AND_CAPACITY) {
                MutableComponent amountString = Component.translatable("primalworld.tooltip.liquid.amount.with.capacity", nf.format(milliBuckets), nf.format(capacity));
                tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
            } else if (tooltipMode == TooltipMode.SHOW_AMOUNT) {
                MutableComponent amountString = Component.translatable("primalworld.tooltip.liquid.amount", nf.format(milliBuckets));
                tooltip.add(amountString.withStyle(ChatFormatting.GRAY));
            }
        } catch (RuntimeException e) {
            primalworld.LOGGER.error("Failed to get tooltip for fluid: {}", String.valueOf(e));
        }
